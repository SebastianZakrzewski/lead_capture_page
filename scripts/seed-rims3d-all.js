...
const ROOT_DIR = path.join(__dirname, '../public/konfigurator/dywaniki/3d/romby');
...
function parse3dFile(fileName) {
  // 5os-3d-diamonds-<material>-<border>.webp
  const m = fileName.match(/^5os-3d-diamonds-([^-]+)-([^.]+)\.webp$/);
  if (!m) return null;
...
function buildImagePath(borderFolder, fileName) {
  return `/konfigurator/dywaniki/3d/romby/${borderFolder}/${fileName}`;
}
...
async function run() {
  console.log('🔎 Skanuję wszystkie foldery 3d/romby...');
  ...
  for (const folder of borderFolders) {
    ...
    for (const file of files) {
      const parsed = parse3dFile(file);
      if (!parsed) {
        // ignore unknown file name patterns
        continue;
      }
      // Ensure border suffix in filename matches mapped border token
      // but trust folder as source of truth; imagePath uses original filename.
      allRecords.push({
