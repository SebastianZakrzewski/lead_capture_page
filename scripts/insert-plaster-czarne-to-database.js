...
async function insertPlasterCzarneToDatabase() {
  try {
    ...
    const command = `powershell -Command "Invoke-WebRequest -Uri 'http://localhost:3000/api/carmat/bulk-insert' -Method POST -Headers @{'Content-Type'='application/json'} -Body '${requestBody.replace(/'/g, "''")}'"`;
