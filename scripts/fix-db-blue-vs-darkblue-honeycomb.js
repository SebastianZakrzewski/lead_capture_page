...
async function run() {
  // Pobierz rekordy 3D/plaster z borderColor = 'niebieski'
  ...
  const updates = [];
  for (const r of rows) {
    if (typeof r.imagePath === 'string' && r.imagePath.endsWith('-blue.webp')) {
      const newPath = r.imagePath.replace(/-blue\.webp$/, '-darkblue.webp');
      updates.push({
        id: r.id,
        matType: r.matType,
  ...
  // Batch update (chunked to avoid payload limits)
  const chunkSize = 50;
